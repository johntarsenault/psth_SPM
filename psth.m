function psth(matFileDir,matFile,roiDir,roiFiles,printDir,tpPre,tpPost,colorValues,HPF,numClusters,TR)
...
%format PSTH_PerRun{run#}.roi{roi#}{condition#} =
%matrix(timePoints,presentations)
if numClusters > 1
    [PSTH_PerRun PSTH_label] = determinePSTH_PerRun_PF(matlabbatch,roiDir,roiFiles,TR,tpPre,tpPost,HPF,numClusters);
else
    [PSTH_PerRun PSTH_label] = determinePSTH_PerRun_Serial(matlabbatch,roiDir,roiFiles,TR,tpPre,tpPost,HPF,numClusters);
end
